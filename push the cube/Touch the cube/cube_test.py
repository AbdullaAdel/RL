...
CAMERA_ID = 0  
ARUCO_SIZE = 0.025      # 25mm Marker
CUBE_SIZE_MM = 35.0     # 35mm Physical Cube

# Lift 3mm to avoid floor friction
Z_MARGIN = 0.003        
FIXED_Z_HEIGHT = (CUBE_SIZE_MM / 1000.0) / 2.0 + Z_MARGIN 

# Smoothing (0.1 = Very Smooth, 0.5 = Fast)
SMOOTHING = 0.1 
...
with open("origin_config.json", "r") as f:
    ...
    # NOTE: The calibration script saves in METERS now.
    # So we do NOT divide by 1000 here anymore.
...
def vision_worker():
    ...
    while running:
            ...
            for i, id_val in enumerate(ids):
                if id_val in VALID_MARKER_IDS:
                    ...
                    if success:
                        ...
                        print(f"\r[VISION] Cube World Pos: X={x_m:.3f}m, Y={y_m:.3f}m", end="")
        ...
        cv2.imshow('Digital Twin Vision', frame)
...
def main():
    ...
    # Auto-Resize Geom to match real cube
    ...
    print("[READY] Digital Twin Active.")
